...
a.width/this.settings.image.lowResolution.width();this.settings.image.properties.relation.height=a.height/this.settings.image.lowResolution.height()},deferredFadeOut:function(a){setTimeout(function(){this.fadeOut()},a)},detachEventListener:function(){c(this.element).off();return this},fadeOut:function(){c(this.settings.selector.preview).fadeOut();c(this.settings.selector.window).fadeOut();return this},getPropertiesOfPreview:function(a){var b={height:c(this.settings.selector.window).height()/this.settings.image.properties.relation.height,
width:c(this.settings.selector.window).width()/this.settings.image.properties.relation.width},d=c(this.settings.image.lowResolution).offsetParent().offset();return c.extend(b,{left:a.pageX-b.width/2-d.left,top:a.pageY-b.height/2-d.top})},modifyCursorAppearance:function(a){c(this.element).css("cursor",a);return this},previewRemainsInScope:function(a){var b=c(this.settings.image.lowResolution);return a.pageX<b.offset().left||a.pageX>b.offset().left+b.width()||a.pageY<b.offset().top||a.pageY>b.offset().top+
b.height()?!1:!0},recognizeMouseMovement:function(a){if(this.settings.resource.isLoaded)if(this.previewRemainsInScope(a)){var b=(a.pageX-this.settings.image.lowResolution.offset().left)*this.settings.image.properties.relation.width-c(this.settings.selector.window).width()/2,d=(a.pageY-this.settings.image.lowResolution.offset().top)*this.settings.image.properties.relation.height-c(this.settings.selector.window).height()/2;this.adjustHighResolutionImage(-b,-d).showPreview(this.getPropertiesOfPreview(a))}else this.fadeOut()},
